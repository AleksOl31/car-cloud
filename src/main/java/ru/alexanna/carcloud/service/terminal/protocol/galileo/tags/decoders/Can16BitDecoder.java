...
public class Can16BitDecoder extends TagGroupDecoder {

    public Can16BitDecoder(int tagCode, MonitoringPackage sourceMonitoringPackage) {
        super(tagCode, sourceMonitoringPackage);
    }

    @Override
    public MonitoringPackage decode(ByteBuf byteBuf) {
        return getCan16Bit(byteBuf);
    }

    private MonitoringPackage getCan16Bit(ByteBuf byteBuf) {
        switch (Tag.fromCode(tagCode)) {
            ...
            case CAN_16_BIT_R13:
            case CAN_16_BIT_R14:
                sourceMonitoringPackage.getCan16BitList().add(tagCAN_16_BIT(byteBuf));
                break;
            default:
